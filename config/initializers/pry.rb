...
Pry.config.prompt_name = Rails.application.class.parent_name.underscore
...
if Rails.env.production?
  ...
  env = Pry::Helpers::Text.green(Rails.env.upcase)
end
...
Pry.config.prompt = Pry::Prompt.new(
  ...
  'my custom prompt',
  [
    proc{ |*args| "#{env} #{old_prompt.wait_proc.call(*args)}" },
    proc{ |*args| "#{env} #{old_prompt.incomplete_proc.call(*args)}" },
  ],
)
